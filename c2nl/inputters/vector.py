...
import numpy as np
from c2nl.inputters.constants import edge_type
import time
...
def vectorize(ex, model):
    ...
    code, summary, gnn = ex['code'], ex['summary'], ex['gnn']
    vectorized_ex = dict()
...
def batchify(batch):
    ...
    #     split_matrix = np.split(matrix, len(edge_type) * 2, axis=1)
    #     type_matrixs = []
    #     for type_matrix in split_matrix:
    #         newmatrix = np.pad(type_matrix, ((0, length_pad - len(type_matrix)), (0, length_pad - len(type_matrix))), 'constant',
    #                            constant_values=(0, 0))
    #         type_matrixs.append(newmatrix)
    #     adjacency_matrix.append(np.hstack(type_matrixs))
    # adjacency_matrix = torch.Tensor(adjacency_matrix)  # adjacency_matrix
    # 这部分是在torch上的操作
    for b in batch:
        matrix = b['adjacency_matrix']
        type_matrixs = []
        for i in matrix.split(matrix.shape[0], dim=-1):
            ...
            newmatrix = F.pad(i, p2d, "constant", 0)
            type_matrixs.append(newmatrix)
    ...
    for i in range(batch_size):
        ...
        code_len_rep[i] = lengths_backbone[i]
        if use_src_word:
