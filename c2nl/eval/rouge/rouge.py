...
import numpy as np
from rouge import Rouge as office_rouge
...
def get_office_rouge(predictions, targets):
    assert (sorted(predictions.keys()) == sorted(targets.keys()))
    imgIds = list(predictions.keys())
    pre = []
    ref = []
    for id in imgIds:
        pre.append(predictions[id][0])
        ref.append(targets[id][0])

    rouges = office_rouge()
    try:
        scores = rouges.get_scores(hyps=pre, refs=ref, avg=True)
