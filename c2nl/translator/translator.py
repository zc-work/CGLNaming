...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs):
        ...
        # by zhou
        nodes = batch_inputs['nodes']  # torch.Size([16, 150])
        adjacency_matrix = batch_inputs['adjacency_matrix']  # torch.Size([16, 150, 1200])
        backbone_sequence = batch_inputs['backbone_sequence']
        max_lengths = batch_inputs['max_lengths']
        lengths_backbone = batch_inputs['lengths_backbone']
        ...
        if model_name == 'Transformer':
            ...
            # by zhou add gnn
            nodes_feature = embedder(nodes, None, mode='gnn')  # torch.Size([16, 150, 512])
            gnn_output = gnn(nodes_feature, adjacency_matrix)  # torch.Size([16, 150, 512])
            batch_input = []
            ...
            gnn_out = torch.stack(batch_input, 0)
            # memory_bank = torch.cat((memory_bank, encoder_input), -1)
            # memory_bank = fuse(memory_bank)
            # end zhou
