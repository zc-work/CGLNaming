...
from c2nl.models.ggnn import GGNN
...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        self.gnn_input_size = args.emsize
        ...
        self.gnn_word_embeddings = Embeddings(args.emsize,
                                              args.gnn_vocab_size,
                                              constants.PAD)
        if self.use_src_word:
            ...
            #                                       args.src_vocab_size,
            #                                       constants.PAD)
            self.src_word_embeddings = self.gnn_word_embeddings
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            ...
            word_rep = self.gnn_word_embeddings(sequence.unsqueeze(2))  # B x P x d
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        # self.gnn.double()
        # self.sbt = GNNEncoder(args, 1, self.embedder.gnn_input_size)

        self.embedder = Embedder(args)
        self.fuse = nn.Linear(self.embedder.enc_input_size * 2, self.embedder.dec_input_size)
    ...
    def _run_forward_ml(self,
        ...
        nodes = kwargs['nodes']  # torch.Size([16, 150])
        ...
        # max_lengths = kwargs['max_lengths'][-1]
        lengths_backbone = kwargs['lengths_backbone']

        code_rep = self.embedder(code_word_rep,  # todo 此处不应该是nodes
                                 None,
                                 mode='encoder')
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len)  # B x seq_len x h

        nodes_feature = self.embedder(nodes, None, mode='gnn')  # torch.Size([16, 150, 512])
        gnn_output = self.gnn(nodes_feature, adjacency_matrix)  # torch.Size([16, 150, 512])
        batch_input = []
        for i in range(batch_size):
            backbone = backbone_sequence[i,:lengths_backbone[i]]
            tmp = torch.index_select(gnn_output[i], 0, backbone)
            if len(backbone) < max_lengths:
                a = torch.zeros((max_lengths-len(backbone), 512)).cuda()
                tmp = torch.cat((tmp, a), 0)
            batch_input.append(tmp)
        # encoder_input = torch.stack(batch_input, 0)
        gnn = torch.stack(batch_input, 0)

        # todo 在这里输入gnnencoder，然后两个encoder的输出输入到decoder中
        # memory_bank = torch.cat((memory_bank, encoder_input), -1)
        # memory_bank = self.fuse(memory_bank)
        ...
        # memory_bank, layer_wise_outputs = self.encoder(encoder_input, code_len)  # B x seq_len x h
    ...
    def decode(self,
        ...
        #在这里输入gnnencoder，然后两个encoder的输出输入到decoder中
        # memory_bank = torch.cat((memory_bank, encoder_input), -1)
        # memory_bank = self.fuse(memory_bank)
