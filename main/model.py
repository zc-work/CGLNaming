...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, args, src_dict, tgt_dict, gnn_dict=None, state_dict=None):
        # Book-keeping.
        self.args = args
        ...
        self.gnn_dict = gnn_dict
        self.args.gnn_vocab_size = len(gnn_dict) if gnn_dict is not None else 0
    ...
    def update(self, ex):
        ...
        nodes = ex['nodes_feature']
        adjacency_matrix = ex['adjacency_matrix']
        backbone_sequence = ex['backbone_sequence']
        lengths_backbone = ex['lengths_backbone']
        max_lengths = max(lengths_backbone).item()
        # max_lengths = torch.tensor([2, max_lengths])
    ...
    def load(filename, new_args=None):
        ...
        gnn_dict = saved_params['gnn_dict']
        state_dict = saved_params['state_dict']
        args = saved_params['args']
        if new_args:
            args = override_model_args(args, new_args)
        return Code2NaturalLanguage(args, src_dict, tgt_dict, gnn_dict=gnn_dict, state_dict=state_dict)
    ...
    def load_checkpoint(filename, use_gpu=True):
        ...
        args = saved_params['args']
        model = Code2NaturalLanguage(args, src_dict, tgt_dict, gnn_dict=gnn_dict, state_dict=state_dict)
