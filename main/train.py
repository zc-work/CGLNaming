...
from c2nl.eval.rouge import Rouge, get_office_rouge
...
def add_train_args(parser):
    ...
    files.add_argument('--train_gnn', nargs='+', type=str,
                       help='Preprocessed train gnn file')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        for i in range(num_dataset):
            ...
            train_gnn = os.path.join(data_dir, args.train_gnn[i])
            ...
            args.train_gnn_files.append(train_gnn)
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    # 207811
    # gnn_dict = util.build_word_and_char_dict(args,
    #                                          examples=train_exs + dev_exs,
    #                                          fields=['gnn'],
    #                                          dict_size=None,
    #                                          no_special_token=True)
    # with open("../../data/java/graph_node.dict", 'wb') as file:
    #     pickle.dump(gnn_dict, file)
    #     print('Graph dictionaries saved done!')
    graph_voc_path = os.path.join(args.data_dir, args.dataset_name[0], "graph_node.dict")
    with open(graph_voc_path, 'rb') as file:
        gnn_dict = pickle.load(file)
        print('Graph dictionaries loaded.')

    # count = 0
    # ast_keys = src_dict.tok2ind.keys()
    # for key in ast_keys:
    #     if key not in gnn_dict.tok2ind:
    #         count = count + 1
    # print('there are ', count , ' words in src dict but not in graph dict') # 780

    logger.info('Num words in graph = %d' % (len(gnn_dict)))  # by zhou
    # traget28239 source34131(uncase 21082)
    logger.info('Num words in source = %d and target = %d' % (len(src_dict), len(tgt_dict)))

    # Initialize model
    model = Code2NaturalLanguage(config.get_model_args(args), src_dict, tgt_dict, gnn_dict)
...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    bleu, rouge_l, meteor, precision, recall, f1, offical_rouge_1, offical_rouge_2, offical_rouge_l = eval_accuracies(hypotheses,
                                                                   references,
                                                                   copy_dict,
                                                                   sources=sources,
    ...
    result['f1'] = f1
    result['or1'] = offical_rouge_1
    result['or2'] = offical_rouge_2
    result['orl'] = offical_rouge_l

    if mode == 'test':
        logger.info('test valid official: '
                    'bleu = %.2f | rouge_l = %.2f | meteor = %.2f | ' %
                    (bleu, rouge_l, meteor) +
                    'Precision = %.2f | Recall = %.2f | F1 = %.2f | rouge1 = %.3f | rouge2 = %.3f | rougel = %.3f |'
                    'examples = %d | ' %
                    (precision, recall, f1, offical_rouge_1, offical_rouge_2, offical_rouge_l, examples) +
        ...
        logger.info('dev valid official: Epoch = %d | ' %
                    ...
                    'Precision = %.2f | Recall = %.2f | F1 = %.2f | rouge1 = %.3f | rouge2 = %.3f | rougel = %.3f | '
                    'examples = %d | ' %
                    (bleu, rouge_l, precision, recall, f1, offical_rouge_1, offical_rouge_2, offical_rouge_l, examples) +
...
def eval_accuracies(hypotheses, references, copy_info, sources=None,
    ...
    # get office_rouge scores
    offical_rouge_1,  offical_rouge_2, offical_rouge_l = get_office_rouge(hypotheses, references)
...
#         meteor = 0
    ...
    return bleu * 100, rouge_l * 100, meteor * 100, precision.avg * 100, \
           recall.avg * 100, f1.avg * 100, offical_rouge_1 * 100, offical_rouge_2 * 100, offical_rouge_l * 100
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, train_gnn, dataset_name in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files, args.train_gnn_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['tgt'] = train_tgt
            # train_files['gnn'] = "../../data/java/train/train.0.gz"
            train_files['gnn'] = train_gnn
    ...
    for dev_src, dev_src_tag, dev_tgt, dev_gnn, dataset_name in \
        ...
        # dev_files['gnn'] = "../../data/java/dev/dev.0.gz"
